...
for topic in user-actions content-interactions; do
  echo "--- ${topic} ---"
  # Avoid `... | head` under pipefail: describe dumps one line per partition
  # and head closing early yields SIGPIPE (exit 141).
  detail="$(docker exec streamsocial-kafka-1 kafka-topics \
