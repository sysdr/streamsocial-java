 ...
 * {@link KafkaTemplate}, publishing onto {@code user-actions} (created
 * Day 3) with the exact {@link UserActionEvent} shape defined Day 1.
 *
 * <p>The controller blocks on the send's future with a short timeout
 * before responding. That's a deliberate choice, not an oversight: the
 * decoupling event-driven architecture buys StreamSocial is between this
 * service and whatever consumes {@code user-actions} downstream (Day 5
 * onward) - not between this endpoint and the broker it's directly
 * talking to. A caller getting back a 202 with a real partition and
 * offset knows the event is durably on the log, not just accepted into
 * an in-memory buffer.
 */
@RestController
