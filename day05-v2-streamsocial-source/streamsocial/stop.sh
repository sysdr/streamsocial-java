...
pkill -f "streamsocial-dashboard.jar" 2>/dev/null && \
  echo "==> Stopped streamsocial-dashboard" || true
pkill -f "streamsocial-producer-service.jar" 2>/dev/null && \
  echo "==> Stopped streamsocial-producer-service" || true
pkill -f "streamsocial-engagement-consumer.jar" 2>/dev/null && \
  echo "==> Stopped streamsocial-engagement-consumer" || true
