...
Day 3 creates `user-actions` and `content-interactions` from
`streamsocial-common`'s `TopicBootstrapDemo`, not from this module. This
script independently checks what landed on the cluster using the broker's
own CLI tools:
...
**Expected output:** both topics listed, with `user-actions` showing its
target partition count (1000 in production, or a smaller local-demo-scale
override if one was used) and `content-interactions` showing its own.
